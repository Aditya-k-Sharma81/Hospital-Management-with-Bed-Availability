...
//         console.log(reversed.docData.address.line1 + " " + reversed.docData.address.line1);
//       }
  ...
  const getUserAppointments = async () => {
    try {
      ...
      if (data.success) {
        ...
        const fixed = reversed.map(app => {
          let address = app.docData.address;

          // If address is a string, convert it into valid JSON
          if (typeof address === "string") {
            try {
              address = JSON.parse(
                address
                  .replace(/(\w+):/g, '"$1":')   // convert line1: → "line1":
                  .replace(/'/g, '"')           // convert 'text' → "text"
              );
            } catch (err) {
