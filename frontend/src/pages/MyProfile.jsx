...
// };
  ...
  return userData && (
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      ...
      <hr className='bt-zinc-400 h-[1px] border-none'/>
      ...
      <div>
        ...
        <div className="grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700">
          ...
          {
            isEdit ? <input className='max-w-25 bg-gray-100' type="date" value={userData.dob} onChange={(e)=> setuserData(prev => ({...prev, dob : e.target.value}))} /> : <p className='text-gray-400'>{userData.dob}</p>
          }
        </div>
      </div>
      <div className='mt-10'>
        {
          isEdit ? <button className='border border-[#5f6FFF] px-8 py-2 rounded-full cursor-pointer hover:bg-[#5f6FFF] hover:text-white transition-all' onClick={updateUserProfileData}>Save information</button> : <button className='border border-[#5f6FFF] px-8 py-2 rounded-full cursor-pointer hover:bg-[#5f6FFF] hover:text-white transition-all' onClick={()=> setIsEdit(true)}>Edit</button>
        }
      </div>
    </div>
  )
